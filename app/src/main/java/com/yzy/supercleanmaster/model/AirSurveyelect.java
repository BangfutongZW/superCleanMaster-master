
package com.yzy.supercleanmaster.model;

public class AirSurveyelect {
	
	private int ID;
	private String dianliuA;
	private String dianliuB;
	private String dianliuC;
	private String dianliang;
	private String wenduldj;
	private String wenduldc;
	private String wendulqj;
	private String wendulqc;
	private String date;
	private String time;
	private int machine;
	public int getID() {
		return ID;
	}
	public void setID(int iD) {
		ID = iD;
	}
	public String getDianliuA() {
		return dianliuA;
	}
	public void setDianliuA(String dianliuA) {
		this.dianliuA = dianliuA;
	}
	public String getDianliuB() {
		return dianliuB;
	}
	public void setDianliuB(String dianliuB) {
		this.dianliuB = dianliuB;
	}
	public String getDianliuC() {
		return dianliuC;
	}
	public void setDianliuC(String dianliuC) {
		this.dianliuC = dianliuC;
	}
	public String getWenduldj() {
		return wenduldj;
	}
	public void setWenduldj(String wenduldj) {
		this.wenduldj = wenduldj;
	}
	public String getWenduldc() {
		return wenduldc;
	}
	public void setWenduldc(String wenduldc) {
		this.wenduldc = wenduldc;
	}
	public String getWendulqj() {
		return wendulqj;
	}
	public void setWendulqj(String wendulqj) {
		this.wendulqj = wendulqj;
	}
	public String getWendulqc() {
		return wendulqc;
	}
	public void setWendulqc(String wendulqc) {
		this.wendulqc = wendulqc;
	}
	public String getDianliang() {
		return dianliang;
	}
	public void setDianliang(String dianliang) {
		this.dianliang = dianliang;
	}
	public String getDate() {
		return date;
	}
	public void setDate(String date) {
		this.date = date;
	}
	public String getTime() {
		return time;
	}
	public void setTime(String time) {
		this.time = time;
	}
	public int getMachine() {
		return machine;
	}
	public void setMachine(int machine) {
		this.machine = machine;
	}
	
	
	

}
